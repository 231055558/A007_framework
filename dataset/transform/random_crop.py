...
class Random_Roi_Crop(BaseTransform):
    def __init__(self, output_size=None, pad_if_needed=False, padding=None, pad_val=0):
        super().__init__()
        self.output_size = output_size
        self.pad_if_needed = pad_if_needed
    ...
    def get_crop_params(self, img, image_path):
        ...
        if "left" in image_path:
            ...
            start_h += random_offset_h
            end_h += random_offset_h
            start_w += random_offset_w
            end_w += random_offset_w
        ...
        # 如果 image_path 包含 "right" 并且输出尺寸为 (512, 512)
            ...
            # 随机微调裁剪区域（32x32 范围）
            random_offset_h = random.randint(-16, 16)  # 随机偏移高度
            random_offset_w = random.randint(-32, 0)   # 随机偏移宽度
            ...
            print("warning")
            # 默认随机裁剪
            start_h = random.randint(0, h - output_h)
            start_w = random.randint(0, w - output_w)
            end_h = start_h + output_h
            end_w = start_w + output_w
            return start_h, start_w, end_h, end_w
