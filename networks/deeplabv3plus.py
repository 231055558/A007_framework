...
from blocks.head import AttentionFC, MultiHeadDiseaseClassifier, MutualRestraintHead
...
class DeepLabV3PlusClassifierMutualHeadOutputMerge(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        # 不设置 aux_loss，默认使用 True（与预训练权重兼容）
        self.model = deeplabv3_resnet50(aux_loss=False, num_classes=256)
        ...
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        # self.fc = nn.Linear(256, num_classes)  # ASPP 输出通道数为 256
        self.fc = MutualRestraintHead(in_features=256, num_classes=num_classes)
